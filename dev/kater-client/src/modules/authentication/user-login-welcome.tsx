...
function WelcomeContent() {
    ...
    const appMainGlobalState = useHookstate(appMainHookState)
    useEffect(()=>{
        appMainGlobalState.dialog.title.set(`Welcome ${appMainGlobalState.appUser.uid.get()}`)
    })
    return (
        <Box sx={getStyles()}>
            <List disablePadding component='div'>
                <ListItem key='1' disableGutters divider disablePadding sx={{border:'1px solid lightGrey', mb:theme.spacing(2)}}>
                    <ListItemButton>
                        <ListItemIcon>
                            <PersonIcon />
                        </ListItemIcon>
                ...
                <ListItem key='2' disableGutters divider disablePadding sx={{border:'1px solid lightGrey'}}>
                    <ListItemButton>
                        <ListItemIcon>
                            <PasswordIcon />
                        </ListItemIcon>
            ...
            <Button sx={{mt:theme.spacing(2)}} variant='contained' onClick={handleSubmit}>Log out</Button>
        </Box>
    )
    function getStyles() {
        const styles: CSSProperties = {
